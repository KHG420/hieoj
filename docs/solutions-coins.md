...
- 没有审核通过题解时，本题题解页按提交编号升序选取一份有非空用户源码的 AC 提交，标注“无人提交题解，选取一份 AC 代码做参考。”，正文只展示原代码。待审核或驳回投稿不影响备用展示；正式题解审核通过后，备用代码自动退出展示。没有可用 AC 源码则保留空状态。
- 备用代码沿用本题阅读与 5 金币解锁规则，解锁后也能阅读后续正式题解。备用内容按访问动态查询，不创建投稿、不奖励金币、不修改历史提交，也不需要数据库迁移。
