...
3. **预览**：显示源学院 / 班级数、新增 / 更新 / 保留、冲突样例与受影响学院清单、
   前若干条变更样例（总数真实，不罗列全部班级）。**预览绝不写库**。若数据库尚未完成
...
# 追加真实完整快照两次 apply 幂等场景
ACADEMIC_ITEST_SNAPSHOT=/path/to/live-directory.json \
  scripts/academic-directory-sync/mysql_integration_test.sh
```

离线测试覆盖查询语义、去重 / 历史行处理、快照校验（`total` 必填）、计划幂等与冲突、
反向映射冲突、无筛选 `getClass` 返回空、`users` 不被触碰。
管理员同步离线测试覆盖 base64 协议、真实格式学院 / 班级解析、完整快照构建、同源跳转边界、
登录页拒绝、截断分页拒绝、权限 / CSRF / challenge / preview nonce 过期拒绝，以及在注入
fake 传输下的完整采集编排（采集成功后清理远端 cookie、密码不进入快照 / session 状态）。
...
`GET_LOCK` 并发、InnoDB 事务回滚、半迁移拒绝、引擎 / 索引 / 字段宽度检查，并可追加完整
`22 学院 / 4141 班级` 快照的两次 apply 幂等场景；脚本结束时销毁自己创建的容器、网络与
