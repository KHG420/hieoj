...
本次统一部署先在隔离环境验证。**没有切换、重启或删除现有生产容器、源代码和数据。**
...
6. 不要把生产 Web 源码覆盖进新容器；代码以仓库构建镜像为准。
   ...
   新镜像保留 Nginx 静态反爬、限流和安全规则；旧主机的动态封禁/网段同步 timer
   仍指向旧路径，不能直接用于新栈。相关脚本完整归档在 `docs/legacy/production-scripts/`；
   正式切换前需单独核对并适配这些运维任务，不能把仓库整理视为线上运维已迁移。
...
同时备份 db-data、credentials、judge-data、uploads；judge-runtime 可备份以保留诊断日志。
