...
echo "Network fail, retry... you might want to make sure https://hub.docker.com/ is available"
echo "Docker image failed, try download from temporary site ... "
# 只使用 dl.hustoj.com 服务器下载，优先系统版本专属包，失败则通用包
while ! wget -O hustoj.docker.tar.bz2 http://dl.hustoj.com/docker/hustoj.docker.$OSRS.tar.bz2; do
    echo "Download system version package failed, trying general version ..."
	if wget -O hustoj.docker.tar.bz2 http://dl.hustoj.com/docker/hustoj.docker.tar.bz2; then
        break
    fi
    echo "All download from dl.hustoj.com failed, retry after 5 seconds..."
	sleep 5s
done
