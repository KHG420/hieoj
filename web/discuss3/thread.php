...
<?php if ($isadmin){
	?><div style="font-size:80%; float:right"> Change sticky level to<?php $adminurl = "threadadmin.php?target=thread";
	if ($row['top_level'] == 0){
		echo "[ ".oj_admin_post_form($adminurl, array('tid'=>$tid,'action'=>'sticky','level'=>'3'), 'Level Top')." ] ";
		echo "[ ".oj_admin_post_form($adminurl, array('tid'=>$tid,'action'=>'sticky','level'=>'2'), 'Level Mid')." ] ";
		echo "[ ".oj_admin_post_form($adminurl, array('tid'=>$tid,'action'=>'sticky','level'=>'1'), 'Level Low')." ] ";
	} else {
		echo "[ ".oj_admin_post_form($adminurl, array('tid'=>$tid,'action'=>'sticky','level'=>'0'), 'Standard')." ] ";
	}
	?> | <?php if ($row['status'] != 1) echo (" [ ".oj_admin_post_form($adminurl, array('tid'=>$tid,'action'=>'lock'), 'Lock')." ]"); else echo(" [ ".oj_admin_post_form($adminurl, array('tid'=>$tid,'action'=>'resume'), 'Resume')." ]");
	?> | <?php echo (" [ ".oj_admin_post_form($adminurl, array('tid'=>$tid,'action'=>'delete'), 'Delete')." ]");
	?></div><?php }
?>
	...
	<td>
     ...
     <div style="display:inline;text-align:left; float:left; margin:0 10px"><a href="../userinfo.php?user=<?php echo $row['author_id']?>"><?php echo $row['author_id']; ?> </a> @ <?php echo $row['time']; ?></div>
		<div class="mon" style="display:inline;text-align:right; float:right">
			...
			<span>[ <?php echo oj_admin_post_form("threadadmin.php?target=reply", array('rid'=>$row['rid'],'tid'=>$tid,'action'=>($row['status']==0?'disable':'resume')), ($row['status']==0?'Disable':'Resume')); ?> ]</span>
			<span>[ <a onclick="reply(<?php echo $row['rid'];?>);">Reply</a> ]</span> 
			<?php } ?>
			<span>[ <a href="#">Quote</a> ]</span>
			<span>[ <a href="#">Edit</a> ]</span>
			<span>[ <?php if ($isuser || $isadmin) echo oj_admin_post_form("threadadmin.php?target=reply", array('rid'=>$row['rid'],'tid'=>$tid,'action'=>'delete'), 'Delete'); ?> ]</span>
		...
		<div id="post<?php echo $row['rid'];?>" class=content style="text-align:left; clear:both; margin:10px 30px">
			<?php	
				if ($row['status'] == -1) echo nl2br(htmlentities($row['content'],ENT_QUOTES,"UTF-8"));
					else{
						if ($row['status'] == 0) echo nl2br(htmlentities($row['content'],ENT_QUOTES,"UTF-8"));
						else {
