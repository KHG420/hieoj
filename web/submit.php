...
if (isset($_POST['id'])) {
    ...
    $sql = "SELECT team FROM `contest` WHERE contest_id=?";
    $team_state = pdo_query($sql, $cid)[0][0];
    if($team_state == 1) {
        $sql = "SELECT COUNT(*) FROM `team` WHERE user_id1=?";
        $res = pdo_query($sql, $user_id);
        if ($res[0][0] == 0) {
            $view_errors= "该比赛为团队赛，请使用队长账号提交。\n";
            require("template/".$OJ_TEMPLATE."/error.php");
...
if((~$OJ_LANGMASK)&(1<<$language)){
    ...
    if ($res[0]['spj'] != '0' && $res[0]['spj'] != '1') {  // 发送到10.0.38.113微服务的提交接口上

//        if ($res[0]['spj'] == '4') {  // cf不准提交重复代码
//            $sql = "SELECT COUNT(*) as num, solution_id FROM `source_code` WHERE `source`=?";
//            $resp = pdo_query($sql, $source);
//            if ($resp['num'] > 0) {
//                $view_errors= "你之前有提交过相同代码。\n";
//                require("template/".$OJ_TEMPLATE."/error.php");
//                exit(0);
//            }
//        }
...
//        $url = "http://127.0.0.1:8000/SubmitOtherOj/";
//        $url = "http://172.31.0.96/mail/SubmitOtherOj/";
...
// 性能修复：按 cache_start 的键结构计算状态页缓存文件并删除，提交后立即能看到自己的新提交
$statusURI="status.php?user_id=".urlencode($_SESSION[$OJ_NAME.'_'.'user_id']);
if (isset($cid))
    $statusURI.="&cid=$cid";
...
$sid.="/".$statusURI;
$sid.=isset($_SESSION[$OJ_NAME.'_'.'OJ_LANG'])?$_SESSION[$OJ_NAME.'_'.'OJ_LANG']:'';
