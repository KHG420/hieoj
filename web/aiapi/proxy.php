...
// 安全修复：代理仅对登录用户开放，防止被当作免费中转滥用（apiKey 配置保持不变）
if (!isset($_SESSION[$OJ_NAME.'_'.'user_id'])) {
    http_response_code(401);
    echo json_encode(array('error' => array('message' => 'Unauthorized', 'type' => 'proxy_error', 'code' => 'UNAUTHORIZED')));
    exit(0);
}
...
class QwenProxy
{
    ...
    public function handleProxyRequest()
    {
	    ...
	    pdo_query("insert into ai_proxy_log(ip,request,response,referer) values(?,?,?,?)",$ip,$requestBody,$response['content'],isset($_SERVER['HTTP_REFERER'])?$_SERVER['HTTP_REFERER']:'');
...
// 配置和启动代理
// 安全修复：CORS 仅允许本站来源（不再使用 *）
$allowed_origin = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']!=='off' ? 'https://' : 'http://').$_SERVER['HTTP_HOST'];
