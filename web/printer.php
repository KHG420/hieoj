...
if ($_SERVER['REQUEST_METHOD']=="POST"){
 ...
 if(isset($OJ_PRINTER)&&$OJ_PRINTER){
	...
	if(isset($_SESSION[$OJ_NAME.'_'.'printer'])){
		// 安全修复：标记完成仅接受 POST（令牌已由上方 check_post_key 校验）；school 通配符转义
		...
		if(isset($_POST['clean'])){
			pdo_query("delete from printer where user_id like ?",$school_like);
		}
		$view_printer=Array();
		$result=pdo_query("select printer_id,user_id,status,content from printer where user_id like ? order by status,printer_id desc limit 50",$school_like);
		$i=0;
