...
// 题目元信息（提交页右侧栏展示）
$view_pmeta = array('title'=>"", 'time_limit'=>0, 'memory_limit'=>0, 'submit'=>0, 'accepted'=>0, 'source'=>"");
if(isset($problem_id) && $problem_id>0){
	$sql="SELECT title,time_limit,memory_limit,submit,accepted,source FROM `problem` WHERE `problem_id`=?";
	$res=pdo_query($sql,$problem_id);
	if(!empty($res)) $view_pmeta=$res[0];
}
/////////////////////////Template
