...
$sql="SELECT `IPprivate` FROM `contest` WHERE `contest_id`=?";
$res = pdo_query($sql,$cid);
$IPprivate = ($res && isset($res[0][0])) ? $res[0][0] : "0";
...
if($OJ_MEMCACHE){
	...
	$sql="SELECT
        ...
        for ($i = 0; $i < $rows_cnt; $i ++) {
                $row = $result[$i];
                $sim_cnt[$i] = isset($sim_map[$row['user_id']])?$sim_map[$row['user_id']]:0;
                $link_cnt[$i] = isset($link_map[$row['user_id']])?$link_map[$row['user_id']]:0;
        }
}
...
for ($i=0;$i<$rows_cnt;$i++){
        ...
        if (strcmp($user_name,$n_user)){
                ...
                $U[$user_cnt]->sim_num=$sim_cnt[$i];
                $U[$user_cnt]->link_num=$link_cnt[$i];
...
$teams = array();
if($team_state == 1) {
        $sql = "SELECT team_name,user_id1,nick1,user_id2,nick2,user_id3,nick3 FROM `team` WHERE user_id1=?";
        for ($i=0;$i<$user_cnt;$i++){
                $res = pdo_query($sql, $U[$i]->user_id);
                $teams[$i] = !empty($res[0]) ? $res[0] : array();
        }
}
