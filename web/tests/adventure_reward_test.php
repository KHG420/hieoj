...
function rw_route_state($ids, $start, $cursor = 0, $mode = 'challenge') {
    $problems = array();
    foreach ($ids as $id) $problems[] = array('id'=>intval($id), 'node_name'=>'验证节点');
    return array('route'=>array('problems'=>$problems, 'start'=>intval($start), 'mode'=>$mode, 'node'=>'fixture', 'cursor'=>intval($cursor)));
}
...
try {
    ...
    // letting a non-strict server coerce the kind and pay anyway. Runs before
    // this suite writes any reward row, so narrowing the enum is safe.
    $u13 = rw_user('n'); $users[] = $u13;
    rw_check((int)rw_value("SELECT COUNT(*) FROM coin_ledger WHERE kind='adventure_reward'") === 0, 'No leftover adventure rows before the enum guard check');
    ...
    // AC2: independent sessions, a regenerated route and concurrent requests
    // still leave exactly one ledger row for the account and the day.
    ...
    foreach (array_map('intval', $matches[1]) as $pid) rw_ac($u2, $pid);
    ...
    list($count, $sum) = rw_adventure($u2);
    rw_check($count === 1 && $sum === 10, 'Second independent session with a regenerated route cannot pay again');
    $u3 = rw_user('c'); $users[] = $u3;
    $s3 = rw_session($u3, rw_route_state(array_slice($pids,0,3), time()));
    ...
    $s4a = rw_session($u4b, rw_route_state(array_slice($pids,0,3), time()));
    $s4b = rw_session($u4b, rw_route_state(array_slice($pids,0,3), time()));
    ...
    $preFixSession = rw_route_state(array_slice($pids,0,3), $routeStart);
    ...
    $s6 = rw_session($u6, rw_route_state(array_slice($pids,3,3), strtotime($todayStart)));
    ...
    $s7 = rw_session($u7, rw_route_state(array_slice($pids,0,3), time()));
    ...
    $s8 = rw_session($u8, rw_route_state(array_slice($pids,0,3), time()));
    ...
    $s9 = rw_session($u9, rw_route_state(array_slice($pids,0,3), time(), $cursor));
    ...
    $s10 = rw_session($u10, rw_route_state(array($pids[0],$pids[1],$hiddenPid), time()));
    ...
    $s11 = rw_session($u11, rw_route_state(array_slice($pids,0,3), time()));
