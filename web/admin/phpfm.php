...
if (!function_exists('str_starts_with')) {
    /**
     * 兼容 PHP < 8.0 的 str_starts_with 替代函数
     * @param string $haystack 要检查的字符串
     * @param string $needle 要匹配的开头子串
     * @return bool 是否以指定子串开头
     */
    function str_starts_with(string $haystack, string $needle): bool {
        // 核心逻辑：如果匹配的子串为空，返回true；否则检查开头是否匹配
        if ($needle === '') {
            return true;
        }
...
if(isset($_GET['generate'])){
    ...
    if(file_exists($current_dir."/Gen.py")  || file_exists($current_dir."/Main.c") || file_exists($current_dir."/Main.cc") ){
        $sql = "INSERT INTO solution(problem_id,user_id,in_date,language,ip,code_length,result) VALUES(?,?,NOW(),?,?,?,1)";
        $insert_id = pdo_query($sql, -$pid, $user_id, 6 , $ip, 0 );

        // ********** 修改这里 **********
        // 注释掉原有调用resolveIDs的代码，替换成直观的提示
        echo "$pid 任务已提交，ID：$insert_id<br>";
        echo "正在生成.out文件，请耐心等待（可刷新页面查看文件是否生成）<br>";
        // 移除这行调用不存在函数的代码：echo "<script>window.setTimeout('resolveIDs()',5000);</script>";
    }else{
