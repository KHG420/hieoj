...
<section class="kg-box"><h2><?php echo $editNode?'编辑':'新增'; ?>知识节点</h2><form method="post" class="kg-form"><?php require('../include/set_post_key.php'); ?><input type="hidden" name="action" value="save_node"><input type="hidden" name="id" value="<?php echo $editNode?intval($editNode['id']):0; ?>"><label>节点标识<input aria-label="节点标识" name="slug" value="<?php echo $editNode?htmlentities($editNode['slug'],ENT_QUOTES,'UTF-8'):''; ?>" placeholder="唯一标识，如 shortest-path" required pattern="[a-z0-9][a-z0-9-]{1,63}"></label><label>节点名称<input aria-label="节点名称" name="name" value="<?php echo $editNode?htmlentities($editNode['name'],ENT_QUOTES,'UTF-8'):''; ?>" placeholder="节点名称" required></label><label>所属领域<input aria-label="所属领域" name="domain" value="<?php echo $editNode?htmlentities($editNode['domain'],ENT_QUOTES,'UTF-8'):''; ?>" placeholder="所属领域" required></label><label>排序<input aria-label="排序" name="sort_order" type="number" value="<?php echo $editNode?intval($editNode['sort_order']):10; ?>" placeholder="排序"></label><label class="wide">学习目标与说明<textarea class="wide" aria-label="学习目标与说明" name="description" rows="2" placeholder="学习目标和知识说明"><?php echo $editNode?htmlentities($editNode['description'],ENT_QUOTES,'UTF-8'):''; ?></textarea></label><button class="wide" type="submit"><?php echo $editNode?'保存修改':'新增节点'; ?></button></form><?php if($editNode){ ?><p><a href="knowledge_graph.php">取消编辑</a></p><?php } ?></section>
...
<section class="kg-box"><h2>添加知识关系</h2><p>前置关系方向：先掌握起点知识，再学习终点知识。</p><form method="post" class="kg-form"><?php require('../include/set_post_key.php'); ?><input type="hidden" name="action" value="add_edge"><label>起点（前置知识）<select aria-label="起点（前置知识）" name="source_node_id"><?php foreach($nodes as $n) echo '<option value="'.intval($n['id']).'">'.htmlentities($n['domain'].' / '.$n['name'],ENT_QUOTES,'UTF-8').'</option>'; ?></select></label><label>终点（后续知识）<select aria-label="终点（后续知识）" name="target_node_id"><?php foreach($nodes as $n) echo '<option value="'.intval($n['id']).'">'.htmlentities($n['domain'].' / '.$n['name'],ENT_QUOTES,'UTF-8').'</option>'; ?></select></label><label>关系类型<select aria-label="关系类型" name="relation"><option value="prerequisite">前置知识</option><option value="strong">强关联</option><option value="related">相关</option></select></label><button type="submit">保存关系</button></form></section>
<section class="kg-box"><h2>添加节点—标签映射</h2><form method="post" class="kg-form"><?php require('../include/set_post_key.php'); ?><input type="hidden" name="action" value="add_mapping"><label>知识节点<select aria-label="知识节点" name="node_id"><?php foreach($nodes as $n) echo '<option value="'.intval($n['id']).'">'.htmlentities($n['domain'].' / '.$n['name'],ENT_QUOTES,'UTF-8').'</option>'; ?></select></label><label>标准标签<select aria-label="标准标签" name="category_id"><?php foreach($categories as $c) echo '<option value="'.intval($c['id']).'">'.htmlentities(kg_category_key($c['content-1'],$c['content-2']),ENT_QUOTES,'UTF-8').'</option>'; ?></select></label><button class="wide" type="submit">保存映射</button></form></section>
<section class="kg-box"><h2>添加兼容题目标签</h2><p>用于把历史写法或未进入分类目录的原始标签映射到知识节点，不会修改题目的原始标签。</p><form method="post" class="kg-form"><?php require('../include/set_post_key.php'); ?><input type="hidden" name="action" value="add_alias"><label>知识节点<select aria-label="知识节点" name="node_id"><?php foreach($nodes as $n) echo '<option value="'.intval($n['id']).'">'.htmlentities($n['domain'].' / '.$n['name'],ENT_QUOTES,'UTF-8').'</option>'; ?></select></label><label>完整题目标签<input aria-label="完整题目标签" name="tag_text" maxlength="191" placeholder="题目中的完整标签文本" required></label><button class="wide" type="submit">保存兼容标签</button></form></section>
