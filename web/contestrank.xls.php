...
//		echo "Add $pid $sec $res<br>";
		...
		if ($res!=4){
			...
			$this->time+=$sec+(isset($this->p_wa_num[$pid]) ? $this->p_wa_num[$pid]*1200 : 0);
			if($this->mark==0){
				$this->mark=$mark_base;
			}else{
			...
			$punish=intval((isset($this->p_wa_num[$pid]) ? $this->p_wa_num[$pid] : 0)*$mark_per_punish);
...
$U=array();
 foreach($result as $row){
	...
	if(isset($end_time)&&isset($lock)&&time()<$end_time+3600&&$lock<strtotime($row['in_date'])) {
