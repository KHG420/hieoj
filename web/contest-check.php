...
//echo "Hello World!";
/**
 * 封装一个 PHP 调试函数，在浏览器控制台打印信息
 * @param mixed $data 要打印的变量（支持任意类型：字符串、数组、对象等）
 * @param string $label 可选标签，方便区分不同调试信息
 */
function consoleLog($data, $label = 'PHP 调试信息') {
    // 处理特殊字符，避免 JS 语法错误
    $jsonData = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
    // 输出 JS 代码
    echo "<script>console.log('{$label}:', {$jsonData});</script>";
}
...
if ($rows_cnt > 0) {
    ...
    $subnet = isset($row['subnet']) ? $row['subnet'] : '';
}
$contest_ok = true;
$password = "";
...
if ($rows_cnt == 0 || (isset($row['defunct']) && $row['defunct'] == 'Y' && !isset($_SESSION[$OJ_NAME . '_administrator']))) {
    $view_title = "比赛已经关闭!";
    $contest_ok = false;
} else {

    $view_private = isset($row['private']) ? $row['private'] : 0;
    $view_contest_creator = isset($row['user_id']) ? $row['user_id'] : '';
    if ($password != "" && hash_equals((string)$row['password'], $password))
...
//                      $contest_ok = false;
    ...
    //consoleLog($unixnow, '基础信息');
    //consoleLog($start_time, '基础信息');
    ...
    if (!isset($_SESSION[$OJ_NAME . '_' . 'administrator']) && $unixnow < $start_time) {
        ...
        $view_errors .= "<p>".(isset($view_description)?$view_description:'')."</p>";
        $view_errors .= "<br>";
...
if (!$contest_ok) {
    ...
    $view_errors .= "<input type=hidden name=postkey value='".htmlentities($_SESSION[$OJ_NAME.'_'.'postkey'],ENT_QUOTES,'UTF-8')."'>";
